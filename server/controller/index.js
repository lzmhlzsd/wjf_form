...
module.exports = {
    ...
    async getFormById ( req, res ) {
        try {
            ...
            HELP.log(
                `${logFile} getFormById sql search result: ${JSON.stringify( result )}`
            )
            if ( result.length > 0 ) {
                ...
                data.fields.forEach( m => {
                    for ( var item in m ) {
                        if (
                            ['field_1', 'field_2', 'field_3', 'field_4'].indexOf( item ) < 0
                        ) {
                ...
                const tsql = `create table t_${formID}_unio (${cTableFileds.join(
                    ','
                )}, ${tableFields.join( ',' )}, ${saleFileds.join( ',' )})`
                await HELP.sqlExecute( tsql )
                HELP.log(
                    `${logFile} getFormById table t_${formID}_unio created success`.green
                )
                ...
                let d5 = d4.replace( /\"/g, '\\"' )
                const sql2 = `insert into t_tables (c_id, c_name, c_content, c_desc) values ('${formID}', '${data.name}', '${d5}', null)`
        ...
        try {
            ...
            for ( var item in entry ) {
                ...
                if ( entry[item] instanceof Object ) {
                    ...
                    if ( HELP.isdatetime( entry[item] ) && item != 'info_os' ) {
                        ...
                        tableValue.push(
                            `'${moment( entry[item] ).format( 'YYYY-MM-DD HH:mm:ss' )}'`
                        )
                    } else if (
            ...
            if ( result1[0].count === 0 ) {
                const unioSql = `insert into t_${tableID}_unio (g_c_name, g_c_grade, g_c_class, g_c_sex, g_c_school,
                    ...
                    (${userResultArray.join( ',' )}, ${tableValue.join(
    ','
)}, ${saleResultArray.join( ',' )})`
                HELP.log( `unioSql Insert: ${unioSql}`.yellow )
                await HELP.sqlExecute( unioSql )
                HELP.log(
                    `${logFile} postFormData t_${tableID}_unio insert success`.green
                )
            } else {
                let setArray = []
                    ...
                    if ( sitem !== 'serial_number' ) {
                        if ( entry[sitem] instanceof Object ) {
                            ...
                            if ( HELP.isdatetime( entry[sitem] ) && sitem != 'info_os' ) {
                                HELP.log(
                                    `${logFile} postFormData datetime ${sitem}: ${entry[sitem]}`
                                )
                                setArray.push(
                                    `${sitem} = '${moment( entry[sitem] ).format(
                                        'YYYY-MM-DD HH:mm:ss'
                                    )}'`
                                )
                            } else if (
                ...
                let updateUserResultArray = [
                    ...
                    `g_c_school = '${
                        userResult.length > 0 ? userResult[0].c_school : ''
                    }'`,
                    `g_c_address = '${
                        userResult.length > 0 ? userResult[0].c_address : ''
                    }'`,
                    `g_c_room = '${userResult.length > 0 ? userResult[0].c_room : ''}'`,
                    `g_c_stuendId = '${
                        userResult.length > 0 ? userResult[0].c_stuendId : ''
                    }'`,
                ...
                let updateSaleResultArray = [
                    `s_c_leader = '${
                        saleResult.length > 0 ? saleResult[0].field_19 : ''
                    }'`,
                    `s_c_name = '${saleResult.length > 0 ? saleResult[0].field_1 : ''}'`,
                    `s_c_school = '${
                        saleResult.length > 0 ? saleResult[0].field_2 : ''
                    }'`,
                    `s_c_grade = '${saleResult.length > 0 ? saleResult[0].field_3 : ''}'`,
                    `s_c_phone = '${saleResult.length > 0 ? saleResult[0].field_4 : ''}'`,
                    `s_c_qq = '${saleResult.length > 0 ? saleResult[0].field_12 : ''}'`,
                    `s_c_type = '${saleResult.length > 0 ? saleResult[0].field_14 : ''}'`,
                    `s_c_recode = '${
                        saleResult.length > 0 ? saleResult[0].field_10 : ''
                    }'`
                ]

                const unioSql = `update t_${tableID}_unio set ${updateUserResultArray.join(
                    ','
                )},  ${setArray.join( ',' )}, ${updateSaleResultArray.join(
                    ','
                )} where serial_number = ${entry.serial_number}`
                HELP.log( `unioSql update: ${unioSql}`.yellow )
                await HELP.sqlExecute( unioSql )
                HELP.log(
                    `${logFile} postFormData t_${tableID}_unio update success`.green
                )
            }
            res.send( 200 )
        } catch ( e ) {
        ...
        try {
            ...
            for ( var item in entry ) {
                ...
                if ( entry[item] instanceof Object ) {
                    ...
                    if ( HELP.isdatetime( entry[item] ) && item != 'info_os' ) {
                        HELP.log(
                            `${logFile} postFromDataSale datetime ${item}: ${entry[item]}`
                        )
                        tableValue.push(
                            `'${moment( entry[item] ).format( 'YYYY-MM-DD HH:mm:ss' )}'`
                        )
            ...
            const result1 = await HELP.sqlExecute( sql1 )
            HELP.log( `select t_${tableID} count: ${JSON.stringify( result1 )}` )
            ...
            if ( result1[0].count === 0 ) {
                ...
                await HELP.sqlExecute( unioSql )
                HELP.log(
                    `${logFile} postFromDataSale t_${tableID} insert success`.green
                )
            } else {
                let setArray = []
                for ( var sitem in entry ) {
                    if ( sitem !== 'serial_number' ) {
                        if ( entry[sitem] instanceof Object ) {
                            ...
                            if ( HELP.isdatetime( entry[sitem] ) && sitem != 'info_os' ) {
                                HELP.log(
                                    `${logFile} postFromDataSale datetime ${sitem}: ${entry[sitem]}`
                                )
                                setArray.push(
                                    `${sitem} = '${moment( entry[sitem] ).format(
                                        'YYYY-MM-DD HH:mm:ss'
                ...
                const unioSql = `update t_${tableID} set ${setArray.join(
                    ','
                )} where serial_number = ${entry.serial_number}`
                HELP.log( `unioSql update: ${unioSql}`.yellow )
                await HELP.sqlExecute( unioSql )
                HELP.log(
                    `${logFile} postFromDataSale t_${tableID} update success`.green
                )
            }
            res.send( 200 )
        } catch ( e ) {
